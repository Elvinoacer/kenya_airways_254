    ...
    return (
      <div className="min-h-screen bg-[#fcf9f8]">
        <main className="mx-auto max-w-4xl px-5 py-20 text-center md:px-20">
          ...
          <p className="mt-3 text-[#5e3f3c]">We could not find that help article.</p>
          <Link
            href="/help"
      ...
      <main>
        <section className="bg-[#410001]">
          <div className="mx-auto max-w-5xl px-5 py-14 md:px-20">
            ...
            <p className="mt-8 text-sm font-bold uppercase tracking-widest text-[#ffb4aa]">{article.category}</p>
            <h1 className="mt-3 text-4xl font-bold leading-tight text-white md:text-5xl">{article.title}</h1>
          </div>
        </section>
          ...
          <article className="rounded-xl border border-[#e5e2e1] bg-white p-6 shadow-sm md:p-8">
            ...
            {article.videoUrl && (
              <div className="mt-8 overflow-hidden rounded-xl border border-[#e5e2e1]">
                <iframe title={article.title} src={article.videoUrl} className="h-64 w-full" />
              </div>
            )}
          ...
          <aside className="space-y-4">
            ...
            {related.map((item) => (
              <Link
                key={item.slug}
                href={`/help/${item.slug}`}
                className="block rounded-xl border border-[#e5e2e1] bg-white p-5 text-sm font-semibold text-[#1A1A1A] shadow-sm hover:text-primary"
              >
                {item.title}
              </Link>
            ))}
