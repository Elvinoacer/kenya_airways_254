...
export default function FlightCard({
  ...
  const destinationCity = flight.destinationCity || destinationAirport?.city || flight.destination;
      ...
      <div className="flex flex-col gap-5 p-4 sm:p-6 md:flex-row md:items-center">
        ...
        <div className="flex flex-1 items-center justify-between min-w-0 gap-2 sm:gap-4">
          ...
          <div className="min-w-0 flex-1 text-left">
            ...
            <div className="mt-2 sm:mt-3">
              <p className="text-lg sm:text-xl font-bold text-gray-900 truncate">{flight.origin}</p>
          ...
          <div className="flex w-20 sm:w-28 shrink-0 flex-col items-center px-1">
            ...
            <div className="flex w-full items-center">
              ...
              <span className="material-symbols-outlined mx-1 sm:mx-2 text-[18px] sm:text-[20px] text-[#e71520] rotate-90" aria-hidden="true">
                flight
              </span>
              <div className="h-[2px] flex-1 rounded-full bg-gray-200" />
            </div>
            <span className={`mt-2 whitespace-nowrap rounded-full px-2 py-0.5 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider ${flight.stops === 0 ? "bg-emerald-50 text-emerald-700" : "bg-orange-50 text-orange-700"}`}>
              {stopsText}
            </span>
          </div>
          ...
          <div className="min-w-0 flex-1 text-right">
            ...
            <div className="mt-2 sm:mt-3">
              <p className="text-lg sm:text-xl font-bold text-gray-900 truncate">{flight.destination}</p>
              <p className="truncate text-xs text-gray-500">{destinationCity}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex w-full flex-row items-center justify-between border-t border-gray-100 pt-4 md:w-auto md:min-w-[140px] md:shrink-0 md:flex-col md:items-end md:justify-center md:border-l md:border-t-0 md:pl-5 md:pt-0">
          <div className="text-left md:text-right">
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Price from</span>
      ...
      <footer className="border-t border-gray-100 bg-gray-50/50 px-4 py-3 sm:px-6">
        <div className="flex flex-wrap gap-x-4 gap-y-2 sm:gap-x-6">
          {capacities.map((entry) => {
            ...
            return (
              <div key={entry.code} className="flex items-center gap-1.5 text-[13px] sm:text-sm">
