...
const PORT = Number(
  process.env.SOCKET_SERVER_PORT || process.env.SOCKET_PORT || process.env.PORT || 3001,
);
...
io.on("connection", (socket) => {
  ...
  socket.on("seat_update", (payload = {}) => {
    if (payload.flightId) io.to(flightRoom(payload.flightId)).emit("seat_update", payload);
    io.emit("seat_update", payload);
  });

  socket.on("booking_change", (payload = {}) => {
    const bookingId = payload.bookingId || payload.id;
    if (bookingId) io.to(bookingRoom(bookingId)).emit("booking_change", payload);
  ...
  socket.on("flight_status", (payload = {}) => {
    if (payload.flightId) io.to(flightRoom(payload.flightId)).emit("flight_status", payload);
...
server.listen(PORT, () => {
  console.log(`[socket] Socket.IO server listening on http://localhost:${PORT}`);
